...
## 🚀 Netlify Deployment (Web Admin)

The web-admin is deployed using **Netlify** with a GitHub Actions workflow for CI/CD. Follow these steps for a reliable deployment:

### 1. Prerequisites
- GitHub repository created and code pushed (see `docs/deploy-instructions.md` for script)
- Netlify account
- Netlify Site created and linked to your GitHub repo

### 2. Netlify Configuration
- **Config file:** `netlify.toml` must be at the project root, with correct encoding (UTF-8, no BOM) and Unix line endings (LF). **Do not edit this file unless necessary.**
- **Build settings:**
  - Build command: (none, or as required)
  - Publish directory: `web-admin`
...
- File: `.github/workflows/netlify-deploy.yml` (do not modify unless you know what you're doing)
- Required GitHub repo secrets:
  - `NETLIFY_AUTH_TOKEN` (Netlify personal access token)
  - `NETLIFY_SITE_ID` (from Netlify Site settings > API ID)
- On push to `main`, the workflow will deploy the latest code to Netlify.

### 5. Troubleshooting & Best Practices
- **Never change `netlify.toml` or workflow files without testing.**
- If deployment fails, restore these files to the last known good state (see commit `main@6ee14c8`).
- Common issues: BOM/encoding errors, line endings, misplaced config files.
- See `docs/deploy-instructions.md` for full troubleshooting and setup guide.
